...
export const getShareableDoctors = async () => {
  const authAxios = getAuthAxios();
  const { data } = await authAxios.get(`${API_BASE}/users/shareable_doctors/`);
  return data;
};

export const sharePatient = async (patientId, doctorIds) => {
  const authAxios = getAuthAxios();
  const { data } = await authAxios.post(
    `${API_BASE}/patients/${patientId}/share/`,
    {
